...
import { CapturingStatusEnum, Options, ShotCountSpecifiedIntervalCapture, getShotCountSpecifiedIntervalCaptureBuilder, stopSelfTimer } from '../../modules/theta-client';
...
const ShotCountSpecifiedIntervalCaptureScreen: React.FC<NativeStackScreenProps<RootStackParamList, 'shotCountSpecifiedIntervalCapture'>> = ({
  navigation
}) => {
  ...
  const capture = React.useState<ShotCountSpecifiedIntervalCapture>();
  const onTake = useCallback(async () => {
    ...
    console.log('ShotCountSpecifiedIntervalCapture options: ' + JSON.stringify(captureOptions));
    console.log('ShotCountSpecifiedIntervalCapture builder: ' + JSON.stringify(builder));
    try {
      capture.current = await builder.build();
      setIsTaking(false);
    } catch (error) {
      setIsTaking(false);
      Alert.alert('ShotCountSpecifiedIntervalCapture build error', JSON.stringify(error), [{
        text: 'OK'
      }]);
    }
  }, [interval, shotCount, captureOptions, isTaking, setIsTaking, setCapture]);
  const initCapture = () => {
  ...
  return <SafeAreaView style={styles.safeAreaContainer} edges={['left', 'right', 'bottom']}>
      ...
      <View style={styles.contentContainer}>
        <ScrollView>
          ...
          <InputNumber title="shot count" placeHolder="Input value" value={shotCount} onChange={useCallback(value => {
