...
import { CaptureModeEnum, FilterEnum, Options, PhotoFileFormatEnum, PresetEnum, getPhotoCaptureBuilder, setOptions, stopSelfTimer } from '../../modules/theta-client';
...
const PhotoCaptureScreen: React.FC<NativeStackScreenProps<RootStackParamList, 'photoCapture'>> = ({
  navigation
}) => {
  ...
  const onTakePhoto = useCallback(async () => {
    ...
    try {
      ...
      setIsTaking(false);
      if (url) {
        Alert.alert('takePicture file url : ', url, [{
          text: 'OK'
        }]);
      } else {
      ...
      setIsTaking(false);
      Alert.alert('takePicture error', JSON.stringify(error), [{
        text: 'OK'
      }]);
    }
  }, [interval, captureOptions, setMessage, setIsTaking]);
  ...
  return <SafeAreaView style={styles.safeAreaContainer} edges={['left', 'right', 'bottom']}>
      <View style={styles.topViewContainer}>
        ...
        <View style={styles.bottomViewContainerLayout}>
          <Button style={styles.button} title="take photo" onPress={onTakePhoto} disabled={isTaking} />
      ...
      <View style={styles.contentContainer}>
        <ScrollView>
          ...
          <EnumEdit title={'filter'} option={captureOptions?.filter} onChange={useCallback(filter => {
          setCaptureOptions(prevState => ({
            ...prevState,
            filter
          }));
        }, [setCaptureOptions])} optionEnum={FilterEnum} />
          ...
          <EnumEdit title={'preset'} option={captureOptions?.preset} onChange={useCallback(preset => {
          setCaptureOptions(prevState => ({
            ...prevState,
            preset
          }));
        }, [setCaptureOptions])} optionEnum={PresetEnum} />
